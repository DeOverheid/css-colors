(function (root) {
  ...
  const state = {
    ...
    lightnessSteps: [],
    hueRows: [],
    hueRowOffsets: Object.create(null),
  ...
  function registerHueRows(rows) {
    ...
    state.hueRows.forEach((entry) => {
      ...
      if (!state.hueRowOffsets[name]) {
        state.hueRowOffsets[name] = { light: 0, dark: 0 };
      }
    });
    emitChange();
  }

  function setPrimaryFromSliders({ hue, saturation }) {
    const normalizedHue = utils.normalizeHue(hue);
  ...
  function applyParsedColor(color) {
    ...
    state.primary.lightness = utils.clamp(color.lightness, 0, 100);
  ...
  function setHueRowOffset(name, offsets) {
    ...
    const current = state.hueRowOffsets[name] || { light: 0, dark: 0 };
