(function (root) {
  ...
  if (!state || !utils || !lightnessAdjustment) {
    throw new Error(
  ...
  function renderSampleRows(snapshot) {
    ...
    entries.forEach(([variant, row]) => {
      ...
      const saturation = computeVariantSaturation(
  ...
  function computeVariantSaturation(baseSaturation, variant) {
    const saturation = utils.clamp(baseSaturation, 0, 100);
    if (variant === "grey") {
      return utils.clamp(saturation / 10, 0, 100);
    }
    if (variant === "neutral") {
      return 0;
    }
  ...
  function applySwatchStyle({
    ...
    const textColor = utils.chooseTextColor(roundedLight);

    swatch.style.backgroundColor = `hsl(${roundedHue}deg ${roundedSat}% ${roundedLight}%)`;
